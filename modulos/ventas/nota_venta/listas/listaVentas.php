...
if ($_POST["tipcomp_cod"] == 1) { //
        $sql = "select 
        p.per_nrodoc,
        cc.ven_cod,
        current_date - vc.ven_fecha as difDias,
        vc.ven_tipfac,
        vc.ven_montocuota,
        vc.ven_nrofac,
        vc.cli_cod,
        ...
        where cc.cuencob_estado <> 'ANULADO' and p.per_nrodoc ilike '%$per_nrodoc%' and ((current_date - vc.ven_fecha) <= 7)
        order by cc.ven_cod;";
} else {
        $sql = "select 
        p.per_nrodoc,
        ...
        join clientes c on c.cli_cod = vc.cli_cod
                join personas p on p.per_cod = c.per_cod
        where cc.cuencob_estado <> 'ANULADO' and p.per_nrodoc ilike '%$per_nrodoc%'
        order by cc.ven_cod;";
}
