...
let ahora = new Date();
$("#rut_fecha").val(formatoFecha(ahora));
...
let limpiarCab = () =>{
    ...
    $(".tblcab .body #rut_fecha").each(function(){
...
let grabar = () => {
    $.ajax({
        ...
        data: {
            ...
            tiprut_descri: $("#tiprut_descri").val(),
...
let controlVacio = () => {
    ...
    if ($("#rut_cod").val() == "") {
        ...
        condicion = "i";
    } else if ($("#rut_fecha").val() == "") {
        condicion = "i";
    } else if ($("#tiprut_descri").val() == "") {
        condicion = "i";
    } else if ($("#cliente").val() == "") {
        condicion = "i";
    } else if ($("#ins_cod").val() == "") {
...
let docRutinas = () => {
    if ($("#rut_cod").val() == '') {
        ...
        window.open ("/SysGym/modulos/servicios/rutinas/docRutinas.php?rut_cod=" + $("#rut_cod").val());
    }
}
...
let enviarDoc = () => {
    if ($("#rut_cod").val() == '') {
        ...
        $.ajax({
            method: "POST",
            url: "/SysGym/others/mail/envioRutinas.php",
            data: { 
                rut_cod: $("#rut_cod").val(),
...
/*enviamos por POST a la base de datos los datos cargados los input para grabar un nuevo detalle de rutcripción*/
function grabar2() {
    $.ajax({
        method: "POST",
        ...
        data: {
            rut_cod: $("#rut_cod").val(),
            ejer_cod: $("#ejer_cod").val(),
            rutdet_series: $("#rutdet_series").val(),
            rutdet_repeticiones: $("#rutdet_repeticiones").val(),
            dia_cod: $("#dia_cod").val(),           
            equi_cod: $("#equi_cod").val(),
            tipequi_cod: $("#tipequi_cod").val(),           
...
let controlVacio2 = () => {
    ...
    if ($("#rut_cod").val() == "") {
        ...
        condicion = "i";
    } else if ($("#ejer_descri").val() == "") {
        condicion = "i";
    } else if ($("#equi_descri").val() == "") {
        condicion = "i";
    } else if ($("#rutdet_series").val() == "") {
        condicion = "i";
    } else if ($("#rutdet_repeticiones").val() == "") {
        condicion = "i";
    } else if ($("#tipequi_cod").val() == "") {
        condicion = "i";
    }
        ...
        data: {
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila2(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    tabla += "<td>" + objeto.ejer_descri + "</td>";
                    tabla += "<td>" + objeto.dia_descri + "</td>";
                    tabla += "<td>" + objeto.equi_descri + "</td>";
                    tabla += "<td style='text-align:right;'>" + objeto.rutdet_series + "</td>";
                    tabla += "<td style='text-align:right;'>" + objeto.rutdet_repeticiones + "</td>";
                tabla += "</tr>";
            }
        ...
        url: "controlador.php"
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    tabla += "<td>" + objeto.rut_cod + "</td>";
                    tabla += "<td>" + objeto.rut_fecha + "</td>";
                    tabla += "<td>" + objeto.usu_login + "</td>";
                    tabla += "<td>" + objeto.suc_descri + "</td>";
                    tabla += "<td>" + objeto.cliente + "</td>";
                    tabla += "<td>" + objeto.tiprut_descri + "</td>";
                    tabla += "<td>" + objeto.funcionario + "</td>";
                    tabla += "<td>" + objeto.rut_estado + "</td>";
                tabla += "</tr>";
            }
...
function getClientes() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/rutinas/listas/listaClientes.php",
        data: {
            cliente:$("#cliente").val()
        }
...
function getDias() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/rutinas/listas/listaDias.php",
        data: {
...
//capturamos los datos de la tabla Ejercicio en un JSON a través de POST para listarlo
function getEjercicio() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/rutinas/listas/listaEjercicio.php",
        data: {
            ejer_descri:$("#ejer_descri").val()
        }
        //en base al JSON traído desde el listaEjercicio arrojamos un resultado
    }).done(function(lista) {
        //el JSON de respuesta es mostrado en una lista
        var fila = "";
        //consultamos si el dato tipeado el front-end existe en la base de datos, si es así, se muestra en la lista
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionEjercicio("+JSON.stringify(item)+")'>"+item.ejer_descri+"</li>";
            });
        }
        //enviamos a los input correspondientes de el conjunto de filas
        $("#ulEjercicio").html(fila);
        //le damos un estilo a la lista de Ejercicio
        $("#listaEjercicio").attr("style", "display:block; position:absolute; z-index:3000; width:100%;");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
//seleccionamos el item por su key y enviamos el dato al input correspondiente
function seleccionEjercicio (datos) {
    Object.keys(datos).forEach(key =>{
        $("#"+key).val(datos[key]);
    });
    $("#ulEjercicio").html();
    $("#listaEjercicio").attr("style", "display:none;");
    $(".foc").attr("class", "form-line foc focused");
}


//capturamos los datos de la tabla Equipo en un JSON a través de POST para listarlo
function getEquipo() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/rutinas/listas/listaEquipo.php",
        data: {
            equi_descri:$("#equi_descri").val()
        }
        //en base al JSON traído desde el listaEquipo arrojamos un resultado
    }).done(function(lista) {
        //el JSON de respuesta es mostrado en una lista
        var fila = "";
        //consultamos si el dato tipeado el front-end existe en la base de datos, si es así, se muestra en la lista
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionEquipo("+JSON.stringify(item)+")'>"+item.equi_descri+"</li>";
            });
        }
        //enviamos a los input correspondientes de el conjunto de filas
        $("#ulEquipo").html(fila);
        //le damos un estilo a la lista de Equipo
        $("#listaEquipo").attr("style", "display:block; position:absolute; z-index:3000; width:100%;");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
function seleccionEquipo (datos) {
    ...
    $("#ulEquipo").html();
    $("#listaEquipo").attr("style", "display:none;");
    $(".foc").attr("class", "form-line foc focused");
}
...
function getFuncionarios() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/rutinas/listas/listaFuncionarios.php",
        data: {
            funcionario:$("#funcionario").val()
        }
        ...
        //le damos un estilo a la lista de GUI
        $("#listaFuncionarios").attr("style", "display:block; position:absolute; width:100%;");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
//capturamos los datos de la tabla Tipos de Rutina en un JSON a través de POST para listarlo
function getTipRut() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/rutinas/listas/listaTipRut.php",
        data: {
            tiprut_descri:$("#tiprut_descri").val()
        }
        //en base al JSON traído desde el listaTipRut arrojamos un resultado
    }).done(function(lista) {
        //el JSON de respuesta es mostrado en una lista
        var fila = "";
        //consultamos si el dato tipeado el front-end existe en la base de datos, si es así, se muestra en la lista
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionTipRut("+JSON.stringify(item)+")'>"+item.tiprut_descri+"</li>";
            });
        }
        //enviamos a los input correspondientes de el conjunto de filas
        $("#ulTipRut").html(fila);
        //le damos un estilo a la lista de TipRut
        $("#listaTipRut").attr("style", "display:block; position:absolute; z-index:3000; width:100%;");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
//seleccionamos el item por su key y enviamos el dato al input correspondiente
function seleccionTipRut (datos) {
    Object.keys(datos).forEach(key =>{
        $("#"+key).val(datos[key]);
    });
    $("#ulTipRut").html();
    $("#listaTipRut").attr("style", "display:none;");
    $(".focus").attr("class", "form-line focus focused");
}
