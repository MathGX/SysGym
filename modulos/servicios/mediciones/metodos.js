...
let ahora = new Date();
$("#med_fecha").val(formatoFecha(ahora));
...
let limpiarCab = () =>{
    ...
    $(".tblcab .body #med_fecha").each(function(){
...
let grabar = () => {
    $.ajax({
        ...
        url: "controlador.php",
        data: {
            med_cod: $("#med_cod").val().trim(),
            med_fecha: $("#med_fecha").val().trim(),
            med_estado: $("#med_estado").val().trim(),
...
function formatoTabla() {
    $(".js-exportable").DataTable({
        language: {
            url: "//cdn.datatables.net/plug-med/1.10.16/i18n/Spanish.json",
        },
        dom:
...
let agregar = () => {
    validarDetalle().done(function(respuesta) {
        if (respuesta.validar == 1) {
            alertaLabel("NO SE PUEDEN AGREGAR MAS PARAMETROS, LA MEDICIÓN SE ENCUENTRA ASOCIADA A UN REGISTRO DE EVOLUCIÓN");
            return;
        }
        $("#operacion_det").val(1);
...
let eliminar = () => {
    validarDetalle().done(function(respuesta) {
        if (respuesta.validar == 1) {
            alertaLabel("NO SE PUEDEN ELIMINAR PARAMETROS, LA MEDICIÓN SE ENCUENTRA ASOCIADA A UN REGISTRO DE EVOLUCIÓN");
            return;
        }
        $("#operacion_det").val(2);
        habilitarBotones2(true);
...
function grabar2() {
    $.ajax({
        ...
        data: {
            med_cod: $("#med_cod").val().trim(),
            param_cod: $("#param_cod").val().trim(),
            meddet_cantidad: $("#meddet_cantidad").val().trim(),   
        ...
        data: {
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila2(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    ...
                    tabla += "<td style='text-align:right;'>" + objeto.meddet_cantidad + "</td>";
...
function getClientes() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/mediciones/listas/listaClientes.php",
        data: {
            cliente:$("#cliente").val()
        }
...
function getParametro() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/mediciones/listas/listaParametro.php",
        data: {
