...
where ic.ins_estado = 'ACTIVO'
	and (p.per_nrodoc ilike '%$cliente%' 
	or p.per_nombres||' '||p.per_apellidos ilike '%$cliente%')
...
if (empty($datos)) {
        echo json_encode(
                array(
                        "fila" => "La persona no se encuentra inscripta o el presupuesto no fue aprobado",
                        "true" => true
                )
        );
