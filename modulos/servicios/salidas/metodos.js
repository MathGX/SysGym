...
let ahora = new Date();
$("#sal_fecha").val(formatoFecha(ahora));
...
let limpiarCab = () =>{
    ...
    $(".tblcab .body #sal_fecha").each(function(){
...
let grabar = () => {
    $.ajax({
        ...
        data: {
            sal_cod: $("#sal_cod").val(),
            sal_fecha: $("#sal_fecha").val(),
            sal_motivo: $("#sal_motivo").val(),
            sal_estado: $("#sal_estado").val(),
            cli_cod: $("#cli_cod").val(),
            ins_cod: $("#ins_cod").val(),
            usu_cod: $("#usu_cod").val(),
            suc_cod: $("#suc_cod").val(),
...
let controlVacio = () => {
    ...
    if ($("#sal_cod").val() == "") {
        ...
        condicion = "i";
    } else if ($("#sal_fecha").val() == "") {
        condicion = "i";
    } else if ($("#ins_cod").val() == "") {
        condicion = "i";
    } else if ($("#cliente").val() == "") {
        condicion = "i";
    } else if ($("#sal_motivo").val() == "") {
        condicion = "i";
    } else if ($("#sal_estado").val() == "") {
        condicion = "i";
    }
        ...
        url: "controlador.php"
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    tabla += "<td>" + objeto.sal_cod + "</td>";
                    tabla += "<td>" + objeto.sal_fecha + "</td>";
                    tabla += "<td>" + objeto.suc_descri + "</td>";
                    tabla += "<td>" + objeto.usu_login + "</td>";
                    tabla += "<td>" + objeto.cliente + "</td>";
                    tabla += "<td>" + objeto.ins_cod + "</td>";
                    tabla += "<td>" + objeto.sal_motivo + "</td>";
                    tabla += "<td>" + objeto.sal_estado + "</td>";
                tabla += "</tr>";
            }
...
function getClientes() {
    $.ajax({
        method: "POST",
        url: "/SysGym/modulos/servicios/salidas/listas/listaClientes.php",
        data: {
            cliente:$("#cliente").val()
        }
