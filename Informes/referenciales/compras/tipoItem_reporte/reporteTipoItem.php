...
<head>
    ...
    <title>Reporte tipos de item</title>
</head>
<?php
...
$sql = "select * from tipo_item 
        where tipitem_cod between 
            case
                when $desde = 0 then 1
                else $desde
            end and 
            case 
                when $hasta = 0 then (select max(tipitem_cod) from tipo_item)
                else $hasta
            end
    ...
    <table class="grilla">
        ...
        <tbody>
            <?php foreach ($datos as $fila) { ?>
                <tr>
                    <td>
                        <?php echo $fila['tipitem_cod']; ?>
                    </td>
                    <td>
                        <?php echo $fila['tipitem_descri']; ?>
                    </td>
                    <td>
                        <?php echo $fila['tipitem_estado'] ?>
                    </td>
                </tr>
            <?php } ?>
        </tbody>
...
$dompdf->stream('reporte_tipItem.pdf', array("Attachment" => false));
?>