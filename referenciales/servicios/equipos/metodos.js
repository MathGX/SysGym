...
let agregar = () => {
    ...
    $("#tipequi_cod, #tipequi_descri, #equi_descri").val("");
    $("#equi_estado").val('ACTIVO');
...
let grabar = () => {
    $.ajax({
        ...
        data: {
            ...
            tipequi_descri: $("#tipequi_descri").val()
        },
...
let controlVacio = () => {
    ...
    if ($("#equi_cod").val() == "") {
        condicion = "i";
    } else if ($("#tipequi_descri").val() == "") {
        condicion = "i";
    } else if ($("#equi_descri").val() == "") {
        condicion = "i";
    } else if ($("#equi_estado").val() == "") {
        condicion = "i";
    }
        ...
        url: "controlador.php"
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    tabla += "<td>";
                        tabla += objeto.equi_cod;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.tipequi_descri;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.equi_descri;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.equi_estado;
                    tabla += "</td>";
                tabla += "</tr>";
            }
...
//capturamos los datos de la tabla TipoEquipos en un JSON a través de POST para listarlo
function getTipoEquipos() {
    $.ajax({
        method: "POST",
        url: "/SysGym/referenciales/servicios/equipos/listas/listaTipoEquipos.php",
        data: {
            tipequi_descri:$("#tipequi_descri").val()
        }
        //en base al JSON traído desde el listaTipoEquipos arrojamos un resultado
    }).done(function(lista) {
        ...
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionTipoEquipos("+JSON.stringify(item)+")'>"+item.tipequi_descri+"</li>";
            });
        }
        //enviamos a los input correspondientes del conjunto de filas
        $("#ulTipoEquipos").html(fila);
        //le damos un estilo a la lista de TipoEquipos
        $("#listaTipoEquipos").attr("style", "display:block; position:absolute; z-index:3000; width:100%;");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
function seleccionTipoEquipos (datos) {
    Object.keys(datos).forEach(key =>{
        $("#"+key).val(datos[key]);
    });
    $("#ulTipoEquipos").html();
    $("#listaTipoEquipos").attr("style", "display:none;");
    $(".focus").attr("class", "form-line focus focused");
}