...
let agregar = () => {
    ...
    $("#per_nombres, #per_apellidos, #per_nrodoc, #per_telefono, #per_email, #tipdoc_descri, #tipdoc_cod").val("");
        ...
        url: "controlador.php"
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    tabla += "<td>";
                        tabla += objeto.per_cod;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.per_nombres;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.per_apellidos;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.per_nrodoc;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.tipdoc_descri;
                    tabla += "</td>";
                    tabla += "<td>";
...
//capturamos los datos de la tabla tipo documentos en un JSON a través de POST para listarlo
function getTipoDoc() {
    $.ajax({
        method: "POST",
        url: "/SysGym/referenciales/servicios/personas/listas/listaTipoDoc.php",
        data: {
            tipdoc_descri:$("#tipdoc_descri").val()
        }
        //en base al JSON traído desde el listaTipoDoc arrojamos un resultado
    }).done(function(lista) {
        ...
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionTipoDoc("+JSON.stringify(item)+")'>"+item.tipdoc_descri+"</li>";
            });
        }
        //enviamos a los input correspondientes del conjunto de filas
        $("#ulTipoDoc").html(fila);
        //le damos un estilo a la lista de tipo documentos
        $("#listaTipoDoc").attr("style", "display:block; position:absolute; z-index:3000; width:100%");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
function seleccionTipoDoc (datos) {
    Object.keys(datos).forEach(key =>{
        $("#"+key).val(datos[key]);
    });
    $("#ulTipoDoc").html();
    $("#listaTipoDoc").attr("style", "display:none;");
    $(".focus").attr("class", "form-line focus focused");
}