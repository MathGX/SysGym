...
let getCod = () => {
    $.ajax({
        ...
        $("#suconf_cod").val(respuesta.codigo);
    });
}
...
let agregar = () => {
    ...
    $("#conf_descri, #suc_descri").val("");
    $("#suconf_estado").val('ACTIVO');
...
let grabar = () => {
    $.ajax({
        ...
        url: "controlador.php",
        data: {
            suconf_cod: $("#suconf_cod").val(),
            suc_cod: $("#suc_cod").val(),
            emp_cod: $("#emp_cod").val(),
            conf_cod: $("#conf_cod").val(),
            suconf_estado: $("#suconf_estado").val(),
...
//_______________________________capturamos los datos de la tabla Configuraciones en un JSON a través de POST para listarlo_______________________________
function getConfiguraciones() {
    $.ajax({
        method: "POST",
        url: "/SysGym/referenciales/seguridad/suc_config/listas/listaConfiguraciones.php",
        data: {
            conf_descri:$("#conf_descri").val()
        }
        //en base al JSON traído desde el listaConfiguraciones arrojamos un resultado
    }).done(function(lista) {
        ...
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionConfiguraciones("+JSON.stringify(item)+")'>"+item.conf_descri+"</li>";
            });
        }
        //enviamos a los input correspondientes de el conjunto de filas
        $("#ulConfiguraciones").html(fila);
        //le damos un estilo a la lista de Configuraciones
        $("#listaConfiguraciones").attr("style", "display:block; position:absolute; z-index:3000; width:100%;");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}

//seleccionamos la configuracion por su key y enviamos el dato al input correspondiente
function seleccionConfiguraciones (datos) {
    Object.keys(datos).forEach(key =>{
        $("#"+key).val(datos[key]);
    });
    $("#ulConfiguraciones").html();
    $("#listaConfiguraciones").attr("style", "display:none;");
    $(".focus").attr("class", "form-line focus focused");
}

//_______________________________capturamos los datos de la tabla sucursales en un JSON a través de POST para listarlo_______________________________
function getSucursales() {
    $.ajax({
        method: "POST",
        url: "/SysGym/referenciales/seguridad/suc_config/listas/listaSucursales.php",
        data: {
            emp_cod:$("#emp_cod").val(),
            suc_descri:$("#suc_descri").val()
        }
