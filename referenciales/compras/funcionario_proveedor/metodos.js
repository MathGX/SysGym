...
let getCod = () => {
    $.ajax({
        ...
        $("#funprov_cod").val(respuesta.codigo);
    });
}
...
let agregar = () => {
    ...
    $(".disabledno, #funprov_nro_doc").removeAttr("disabled");
    $(".focus").attr("class", "form-line focus focused");
    $(".disabledno, #funprov_nro_doc").val("");
    $(".tbl").attr("style", "display:none");
    $("#funprov_estado").val('ACTIVO');
    getCod();
...
let sinCaracteres = (nombreInput, idInput) => {
    ...
    if ( valor !== "" && caracteres.test(valor)) {
        mensaje = "El campo <b>" + nombreInput + "</b> no acepta caracteres especiales a parte del guión.";
        alertaLabel(mensaje);
        $(idInput).val("");
    }
}
        ...
        url: "controlador.php"
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    tabla += "<td>" + objeto.funprov_cod + "</td>";
                    tabla += "<td>" + objeto.funcionario + "</td>";
                    tabla += "<td>" + objeto.funprov_nro_doc2 + "</td>";
                    tabla += "<td>" + objeto.pro_razonsocial + "</td>";
                    tabla += "<td>" + objeto.pro_ruc + "</td>";
                    tabla += "<td>" + objeto.funprov_estado + "</td>";
                tabla += "</tr>";
            }
...
function getProveedor() {
    $.ajax({
        method: "POST",
        url: "/SysGym/referenciales/compras/funcionario_proveedor/listas/listaProveedor.php",
        data: {
            pro_razonsocial:$("#pro_razonsocial").val()
        }
