...
let getCod = () => {
    $.ajax({
        ...
        $("#entahd_cod").val(respuesta.codigo);
    });
}
...
let sinCaracteres = (nombreInput, idInput) => {
    ...
    if ( valor !== "" && caracteres.test(valor)) {
        mensaje = "El campo <b>" + nombreInput + "</b> no puede aceptar caracteres especiales, solo puntos.";
        alertaLabel(mensaje);
        $(idInput).val("");
    }
}
        ...
        url: "controlador.php"
            ...
            for (objeto of respuesta) {
                tabla += "<tr onclick='seleccionarFila(" + JSON.stringify(objeto).replace(/'/g, '&#39;') + ")'>";
                    tabla += "<td>";
                        tabla += objeto.entahd_cod;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.martarj_descri;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.ent_razonsocial;
                    tabla += "</td>";
                    tabla += "<td>";
                        tabla += objeto.entahd_estado;
                    tabla += "</td>";
                tabla += "</tr>";
            }
...
//capturamos los datos de la tabla MarcaTarjeta en un JSON a través de POST para listarlo
function getMarcaTarjeta() {
    $.ajax({
        method: "POST",
        url: "/SysGym/referenciales/ventas/entidad_adherida/listas/listaMarcaTarjeta.php",
        data: {
            martarj_descri:$("#martarj_descri").val()
        }
        //en base al JSON traído desde el listaMarcaTarjeta arrojamos un resultado
    }).done(function(lista) {
        ...
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionMarcaTarjeta("+JSON.stringify(item)+")'>"+item.martarj_descri+"</li>";
            });
        }
        //enviamos a los input correspondientes del conjunto de filas
        $("#ulMarcaTarjeta").html(fila);
        //le damos un estilo a la lista de MarcaTarjeta
        $("#listaMarcaTarjeta").attr("style", "display:block; position:absolute; z-index:3000; width:100%");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
function seleccionMarcaTarjeta (datos) {
    Object.keys(datos).forEach(key =>{
        $("#"+key).val(datos[key]);
    });
    $("#ulMarcaTarjeta").html();
    $("#listaMarcaTarjeta").attr("style", "display:none;");
    $(".focus").attr("class", "form-line focus focused");
}

//capturamos los datos de la tabla EntidadEmisora en un JSON a través de POST para listarlo
function getEntidadEmisora() {
    $.ajax({
        method: "POST",
        url: "/SysGym/referenciales/ventas/entidad_adherida/listas/listaEntidadEmisora.php",
        data: {
            ent_razonsocial:$("#ent_razonsocial").val()
        }
        //en base al JSON traído desde el listaEntidadEmisora arrojamos un resultado
    }).done(function(lista) {
        //el JSON de respuesta es mostrado en una lista
        var fila = "";
        //consultamos si el dato tipeado el front-end existe en la base de datos, si es así, se muestra en la lista
        if(lista.true == true){
            ...
            $.each(lista,function(i, item) {
                fila += "<li class='list-group-item' onclick='seleccionEntidadEmisora("+JSON.stringify(item)+")'>"+item.ent_razonsocial+"</li>";
            });
        }
        //enviamos a los input correspondientes del conjunto de filas
        $("#ulEntidadEmisora").html(fila);
        //le damos un estilo a la lista de EntidadEmisora
        $("#listaEntidadEmisora").attr("style", "display:block; position:absolute; z-index:3000; width:100%");
    }).fail(function (a,b,c) {
        swal("ERROR",c,"error");
    })
}
...
//seleccionamos el modulo por su key y enviamos el dato al input correspondiente
function seleccionEntidadEmisora (datos) {
    Object.keys(datos).forEach(key =>{
        $("#"+key).val(datos[key]);
    });
    $("#ulEntidadEmisora").html();
    $("#listaEntidadEmisora").attr("style", "display:none;");
    $(".focus").attr("class", "form-line focus focused");
}